...
#Examine Trawl Sites
```{r}
haul <- pull_haul(survey = "NWFSC.Combo", years = c(2023, 2050))
...
p <- ggplot() +
  ...
  geom_polygon(
    ...
    fill = "white",   # off white land
    color = "gray60"
  ) +
  # trawl points with fill + outline
  geom_point(
    data = trawl_sites,
    aes(x = longitude_dd, y = latitude_dd),
    shape = 21,                 # circle with outline
    fill = "yellow2",           # inside color
    color = "goldenrod4",       # darker outline
    size = 2,
    alpha = 0.2,                # translucency for fill
    stroke = 0.3                # outline thickness
  ) +
  # coordinate system with buffer
  coord_quickmap(
    xlim = range(trawl_sites$longitude_dd, na.rm = TRUE) + c(-2, 2),
  ...
  theme_minimal() +
  theme(
    panel.background = element_rect(fill = "lightblue3", color = NA) # ocean
  )

ggsave("survey_sites_map.png", p, width = 8, height = 6, dpi = 300)
```
...
write_csv(catch_data, "catch_data.csv")
...
catch_data <- catch_data %>%
  ...
  ungroup() %>%
  # Keep species present in >= 50 such trawls
  filter(n_trawls >= 50)

#table(catch_data_test$Common_name)
```

#Species numbers
```{r}
species_trawl_counts <- catch_data %>%
  filter(total_catch_numbers > 0) %>%                 # only positive catches
  group_by(Common_name) %>%
  summarise(n_trawls = n_distinct(Trawl_id), .groups = "drop") %>%
  arrange(desc(n_trawls))   # sort high → low

head(species_trawl_counts, 5)
```
...
# bsb$resids <- residuals(fit) # randomized quantile residuals
# # d$resids <- residuals(m, type = "mle-mcmc") # better but slower
# hist(bsb$resids)
# 
# qqnorm(bsb$resids)
# abline(a = 0, b = 1)
# 
# ggplot(bsb, aes(Lon.km, Lat.km, col = resids)) + scale_colour_gradient2() +
...
for (i in seq_along(species_cols)) {
  ...
  ggsave(
    filename = file.path(outdir, paste0(sp, "_map_plot.png")),
    plot = map_p,
...
#Fix name capitilizations
life_history <- life_history %>%
  mutate(Common_name = str_to_title(Common_name))
unique(life_history$Common_name)

coef_plot_data <- coef_plot_data %>%
  rename(Common_name = species_pretty)


#Final Plot
final_plot_data <- coef_plot_data %>%
  left_join(
    life_history,
    by = "Common_name"
  )
...
final_plot_data <- final_plot_data %>%
  mutate(Common_name = forcats::fct_reorder(Common_name, estimate))

#########################
#Overall Figure of resident/mobile
#########################

resident_mobile_chla <- ggplot(final_plot_data,
       aes(x = estimate, y = Common_name, color = Dominant_movement)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  labs(
    ...
    y = "Species",
    title = "Species-specific effect of chlorophyll-a",
    color = "Dominant movement"  # legend title
  ) +
  theme_minimal()

ggsave(
    filename = "resident_mobile_chla.png",
    plot = resident_mobile_chla,
    width = 9, height = 7, dpi = 400
  )

#########################
#Overall Figure of planktonivores and other
#########################
trophic_level_chla <- ggplot(final_plot_data,
       aes(x = estimate, y = Common_name, color = adult_prey_category)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  labs(
  ...
  theme_minimal()
ggsave(
    filename = "trophic_level_chla.png",
    plot = trophic_level_chla,
    width = 9, height = 7, dpi = 400
  )

trophic_level_chla_detailed <- ggplot(final_plot_data,
       aes(x = estimate, y = Common_name, color = Dominant_prey_adult)) +
  geom_point() +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50") +
  labs(
  ...
  theme_minimal()
ggsave(
    filename = "trophic_level_chla_detailed.png",
    plot = trophic_level_chla_detailed,
    width = 9, height = 7, dpi = 400
  )
