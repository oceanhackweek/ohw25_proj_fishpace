# Create Larval Datasets from Real CalCOFI Data
# Uses the larval stages dataset as primary source for Larvae.csv and LarvaeStages.csv
...
cat("=== Create Larval Datasets from Real CalCOFI Data ===\n")
...
create_larval_datasets <- function() {
  ...
  if (!file.exists(stages_file)) {
    ...
    cat("⚠️  Run the ERDDAP download section below...\n")
    
    # Download CalCOFI larval data using direct download
    download_calcofi_larval_data()
...
main_create_larval_datasets <- function() {
  cat("Starting creation of standardized larval datasets...\n\n")
  ...
  cat("\n🐟 CalCOFI larval datasets created successfully!\n")
  cat("Files: Larvae.csv, LarvaeStages.csv\n")
